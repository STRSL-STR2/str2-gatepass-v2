...
export default function InvoiceRecords() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Fetch gate pass records
      const { data: gpRecords, error: gpError } = await supabase.from('gate_pass_records').select('gate_pass_no, status, rows');
      if (gpError) throw gpError;
      
      // Load master data to enrich missing fields
      let localMasterData: any[] = [];
      const stored = await localforage.getItem("masterData");
      if (stored) {
        localMasterData = stored as any[];
      }
      const masterDataMap = new Map(localMasterData.map(r => [r.invoice, r]));

      const aggregatedMap = new Map<string, MasterDataRow & { is_posted?: boolean }>();
        ...
        for (const gp of gpRecords) {
          ...
          for (const row of rows) {
            ...
            if (aggregatedMap.has(invNo)) {
              ...
              existing.qty_invoiced = Number(existing.qty_invoiced) + Number(row.qty_invoiced || row.mtrs || 0);
              existing.extended_price = Number(existing.extended_price) + Number(row.extended_price || row.value || 0);
              existing.cartons = Number(existing.cartons || 0) + Number(row.cartons || 0);
            } else {
              const mdRow = masterDataMap.get(invNo) || {};
              const newRow = {
                ...mdRow,
                ...row,
                name: row.name || mdRow.name || "-",
                invoice_date: row.invoice_date || mdRow.invoice_date || "-",
                order_no: row.order_no || mdRow.order_no || "-",
                do_bol: row.do_bol || row.do || mdRow.do_bol || "-",
                cust_po: row.cust_po || row.po || mdRow.cust_po || "-",
                ship_via_description: row.ship_via_description || row.buyer || mdRow.ship_via_description || "-",
                consignee_address_3: row.consignee_address_3 || mdRow.consignee_address_3 || "-",
                qty_invoiced: row.qty_invoiced || row.mtrs || 0,
                extended_price: row.extended_price || row.value || 0,
                cartons: Number(row.cartons || 0),
                gate_pass_issued: gp.gate_pass_no,
                is_posted: isPosted,
              };
      ...
      toast.error(`Error loading invoice records: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    const filtered = data.filter(row => {
      ...
      if (searchTerm) {
        ...
        matchesSearch = (
          ...
          (row.cust_po?.toLowerCase() || "").includes(term) ||
          (row.ship_via_description?.toLowerCase() || "").includes(term) ||
          (row.consignee_address_3?.toLowerCase() || "").includes(term)
        );
      }
    ...
    <div className="flex flex-col flex-1 h-full space-y-4 overflow-hidden">
      <div className="flex flex-wrap items-center gap-3 bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm mb-4">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <Button variant="outline" onClick={handleExport} disabled={data.length === 0} className="h-9">
        ...
        <Table className="min-w-[1200px]">
          <TableHeader className="bg-slate-200 dark:bg-slate-800 sticky top-0 z-10 shadow-sm">
            ...
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell colSpan={15} className="h-48 text-center text-muted-foreground">
                ...
                <TableRow>
                  <TableCell colSpan={15} className="h-48 text-center text-muted-foreground font-medium">
                ...
                filteredData.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="text-right font-medium text-xs font-mono">{(Number(row.cartons) || 0).toLocaleString()}</TableCell>
                      ...
                      <TableCell className="text-xs truncate max-w-[200px]" title={row.do_bol}>{row.do_bol}</TableCell>
                      ...
                      <TableCell className="text-center">
                        {row.is_posted ? (
                          <Badge variant="default" className="bg-blue-600 hover:bg-blue-700 mx-auto flex w-max">Posted</Badge>
                        ) : (
                          <span className="text-muted-foreground text-xs">-</span>
                        )}
                      </TableCell>
